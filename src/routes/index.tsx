...
export const Route = createFileRoute("/")({
  ...
  errorComponent: ({ error }) => (
    <div className="flex min-h-screen items-center justify-center p-8 text-center text-sm text-muted-foreground">
      データの読み込みに失敗しました。{error instanceof Error ? error.message : String(error)}
    </div>
  ),
  component: Home,
});
...
// モジュール読み込み時に1度だけ評価すると、常駐するSSRサーバーでは
// 「今日」がプロセス起動日のまま固定されてしまう。呼ぶたびに作る。
const createEmptyDraft = (): CustomGrantInput => ({
  title: "",
  organization: "",
  category: "地域振興",
  amountMin: 0,
...
function Home() {
  ...
  const [draft, setDraft] = useState<CustomGrantInput>(createEmptyDraft);
  ...
  const runSync = async () => {
    setSyncing(true);
    try {
      const res = await fetch("/api/public/hooks/sync-jgrants", { method: "POST" });
      ...
      if (res.status === 429) {
        const min = Math.ceil((body?.retryAfterSec ?? 0) / 60);
        toast.info(`最新の状態です。次の更新まで約${Math.max(min, 1)}分お待ちください。`);
        return;
      }
      if (!res.ok || !body?.ok) {
      ...
      toast.success(
        `jGrantsから${body.count ?? 0}件を取り込みました。` +
          (removed > 0 ? `（募集終了 ${removed}件を削除）` : ""),
      );
      await router.invalidate();
    } catch (e) {
      toast.error(`更新に失敗しました: ${errorText(e)}`);
    } finally {
      setSyncing(false);
    }
  };
  ...
  const filtered = useMemo(() => {
    return allGrants
      .filter((g) => {
        if (
          query &&
          !`${g.title} ${g.organization} ${g.description}`
            .toLowerCase()
        ...
        if (
          month !== "すべて" &&
          !coversMonth(g.applicationStart, g.applicationEnd, parseInt(month, 10))
        )
          return false;
        return true;
      })
  ...
  const openEdit = (g: Grant) => {
    ...
    setDraft({ ...createEmptyDraft(), ...rest });
    setDialogOpen(true);
  };
  const saveDraft = async () => {
    ...
    if (user) {
      try {
        ...
        toast.error(`保存に失敗しました: ${errorText(e)}`);
        return;
      }
    } else if (editingId) {
      setCustoms((prev) =>
        prev.map((g) => (g.id === editingId ? { ...g, ...draft, id: editingId, custom: true } : g)),
      );
    } else {
  ...
  const deleteCustom = async (id: string) => {
    ...
    if (user) {
      try {
        await deleteUserGrant(id);
      } catch (e) {
        toast.error(`削除に失敗しました: ${errorText(e)}`);
        void refreshUserData();
      }
    }
  };
  ...
  const runAgent = async () => {
    ...
    try {
      const compact = allGrants.slice(0, 400).map((g) => ({
        ...
        category: g.category,
        region: getGrantRegion(g),
        amountMin: g.amountMin,
        amountMax: g.amountMax,
        applicationStart: g.applicationStart,
        applicationEnd: g.applicationEnd,
        target: g.target,
      }));
              ...
              <section>
                <div className="mb-4 flex items-baseline justify-between">
                  <h2 className="text-xl font-semibold">
                    {filtered.length}{" "}
                    <span className="text-sm font-normal text-muted-foreground">件の助成金</span>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {filtered.map((g) => (
                      <GrantCard
                        ...
                        onToggle={() => toggleFav(g.id)}
                        onEdit={g.custom ? () => openEdit(g) : undefined}
                        onDelete={g.custom ? () => deleteCustom(g.id) : undefined}
                      />
                    ))}
                  </div>
                )}
          ...
          <TabsContent value="favorites" className="mt-6">
            <div className="mb-6 grid gap-4 sm:grid-cols-3">
              <StatCard
                ...
                label="保存件数"
                value={`${favList.length} 件`}
                tone="warm"
              />
              <StatCard
                icon={<Coins className="h-5 w-5" />}
                label="最大調達可能額"
                value={formatYen(totalFundingSecured)}
                tone="leaf"
              />
              <StatCard
                icon={<Calendar className="h-5 w-5" />}
                label="直近の締切"
                value={favList[0] ? formatDate(favList[0].applicationEnd) : "—"}
                tone="primary"
              />
            </div>
            {favList.length === 0 ? (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={saveDraft}
              disabled={!draft.title.trim() || !draft.organization.trim()}
            >
              保存
            </Button>
          </DialogFooter>
        ...
        <div className="flex items-center gap-2">
          ...
          {email ? (
            ...
            <Button asChild variant="outline" size="sm">
              <Link to="/auth">
                <LogIn className="mr-1 h-4 w-4" /> ログイン
              </Link>
            </Button>
          )}
        </div>
...
function Hero() {
  return (
    <section className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-accent/60 via-background to-warm/25 p-8 shadow-sm sm:p-12">
      <div
        className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-warm/30 blur-3xl"
        aria-hidden
      />
      <div
        className="absolute -bottom-10 -left-10 h-48 w-48 rounded-full bg-leaf/25 blur-3xl"
        aria-hidden
      />
...
function FilterPanel({
  ...
  setAmountRange: (v: [number, number]) => void;
  month: string;
...
function CustomGrantForm({
  draft,
  setDraft,
}: {
  draft: CustomGrantInput;
  setDraft: (d: CustomGrantInput) => void;
}) {
  const upd = <K extends keyof CustomGrantInput>(k: K, v: CustomGrantInput[K]) =>
    setDraft({ ...draft, [k]: v });
  return (
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input
            ...
            onChange={(e) => upd("amountMin", Number(e.target.value) || 0)}
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(e) => upd("amountMax", Number(e.target.value) || 0)}
          />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input
            ...
            onChange={(e) => upd("applicationStart", e.target.value)}
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(e) => upd("applicationEnd", e.target.value)}
          />
        </div>
      </div>
      <div>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-2">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {grant.custom && (
              <Badge className="border-0 bg-primary/15 text-primary">自分の登録</Badge>
            )}
            {urgent && (
        ...
        {grant.url ? (
          <a href={grant.url} target="_blank" rel="noreferrer" className="mt-2 block group">
            <h3 className="text-base font-bold leading-snug group-hover:text-primary group-hover:underline underline-offset-2">
              {grant.title}
            </h3>
          </a>
        ) : (
        ...
        <div className="grid grid-cols-2 gap-2 rounded-xl bg-muted/60 p-3 text-xs">
          <div>
            ...
            <p className="mt-0.5 font-semibold text-foreground">
              {formatYen(grant.amountMin)}
              <span className="text-muted-foreground"> 〜 </span>
              {formatYen(grant.amountMax)}
            </p>
          </div>
          <div>
            <p className="flex items-center gap-1 text-[10px] uppercase tracking-wide text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
            ...
            {grant.url && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex h-7 items-center gap-1 rounded-md bg-primary/10 px-2 text-xs font-medium text-primary hover:bg-primary/20"
              >
...
function AiSearchPanel({
  ...
  if (filters) {
    ...
    if (filters.amountMin > 0 || filters.amountMax < 100_000_000)
      chips.push({
        label: "金額",
        value: `${formatYen(filters.amountMin)}〜${formatYen(filters.amountMax)}`,
      });
    ...
    <div className="mb-6 rounded-2xl border border-primary/30 bg-gradient-to-br from-accent/40 via-card to-warm/20 p-5 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">
            研究分野・所属・応募資格・金額・締切・地域を抽出して検索します。
          </p>
        </div>
      </div>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {chips.map((c, i) => (
            <Badge key={i} variant="secondary" className="bg-primary/10 text-primary border-0">
              <span className="text-[10px] opacity-70 mr-1">{c.label}:</span>
              {c.value}
            </Badge>
          ))}
        </div>
      ...
      {answer && matchedIds.length > matches.length && (
        <p className="mt-2 text-[11px] text-muted-foreground">
          ※ AIが挙げた {matchedIds.length - matches.length}{" "}
          件は現在の検索結果内に見つからないため、出典として表示していません。
        </p>
      )}
